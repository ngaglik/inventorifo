...
namespace Inventorifo.App
{
    ...
    class TransactionSaleReturn : Gtk.Box
    {
        int transaction_type_id = 4;
        ...
        public TransactionSaleReturn(Builder builder) : base(builder.GetRawOwnedObject("TransactionSaleReturn"))
        {
            builder.Autoconnect(this);

            Label lbTitle = (Label)builder.GetObject("LbTitle");
            lbTitle.Text = "Sale Return";
        ...
        {
            DataTable dtTransSelected = CoreCl.fillDtTransactionSaleReturn(transaction_id, btnDate.Label, "");
        ...
        private void NewTransaction(object sender, EventArgs e)
        {
            ...
            "values (4,CURRENT_DATE,CURRENT_DATE,1,"+this.parent.user.id+",'"+this.parent.application_id+"') ";
        ...
        {                  
                ...
                clTransaction tran;
                dtTransSelected =  CoreCl.fillDtTransactionSaleReturn(transaction_id, btnDate.Label, strfind);
                foreach (DataRow dr in dtTransSelected.Rows)
                {              
        ...
        public Response AddItem(){
            clStock sto;
            Console.WriteLine("==============1===="+ dtAddItemSelected.Rows[0].ItemArray[0].ToString()); 
            Response resp = new Response();
            foreach (DataRow drs in  dtAddItemSelected.Rows){
                sto = new clStock{ 
                        product_id = dtAddItemSelected.Rows[0].ItemArray[2].ToString(),
                        quantity = spnQty.Text,
                        expired_date = drs["expired_date"].ToString(),
                        price_id = drs["expired_date"].ToString(),
                ...
                string sql = "insert into transaction_item (transaction_id,product_id,quantity,stock_id,price_id,state, location, condition) "+
        ...
        private void ShowReferencePopup(object sender, EventArgs e)
        {
            ...
            {                   
                ...
                ReportTransactionSale refWidget = new ReportTransactionSale(this,"dialog",filterTrans);
        ...
        private Response DoCheckout()
        {
            ...
            Console.WriteLine(sql);
            DbCl.ExecuteTrans(DbCl.getConn(), sql);
            sql = "update transaction set return_amount=return_amount+" + GetTotalSalePrice().ToString()+", state=0 where id="+lbTransactionId.Text;
            Console.WriteLine(sql);
            DbCl.ExecuteTrans(DbCl.getConn(), sql);
            SetTransactionModel("",entSearch.Text.Trim());  
